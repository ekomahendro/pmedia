    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'add' || $_POST['action'] === 'edit') {
            ...
            if ($ayah && $_POST['ayah_select'] === 'other') {
                $ayah_result = $conn->query("SELECT id FROM anggotaatuk WHERE nama='$ayah'");
                if ($ayah_result->num_rows == 0) {
                    ...
                    if (!$error) {
                        $stmt = $conn->prepare("INSERT INTO anggotaatuk (nama, tempat_lahir, tgl_lahir, domisili, foto) VALUES (?, ?, ?, ?, ?)");
            ...
            if ($ibu && $_POST['ibu_select'] === 'other') {
                $ibu_result = $conn->query("SELECT id FROM anggotaatuk WHERE nama='$ibu'");
            ...
            if (!$error) {
                if ($_POST['action'] === 'add') {
                    $stmt = $conn->prepare("INSERT INTO anggotaatuk (nama, tempat_lahir, tgl_lahir, ayah, ibu, domisili, foto) VALUES (?, ?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("sssssss", $nama, $tempat_lahir, $tgl_lahir, $ayah, $ibu, $domisili, $foto);
                    $stmt->execute();
                    $stmt->close();
                } elseif ($_POST['action'] === 'edit') {
                    $id = $_POST['id'];
                    $stmt = $conn->prepare("UPDATE anggotaatuk SET nama=?, tempat_lahir=?, tgl_lahir=?, ayah=?, ibu=?, domisili=?, foto=? WHERE id=?");
            ...
            $id = $_POST['id'];
            $foto_result = $conn->query("SELECT foto FROM anggotaatuk WHERE id=$id");
            ...
            $stmt = $conn->prepare("DELETE FROM anggotaatuk WHERE id=?");
...
$anggota = $conn->query("SELECT * FROM anggotaatuk $search_query ORDER BY nama")->fetch_all(MYSQLI_ASSOC);
$nodes = [];
$edges = [];
$marriage_nodes = [];
$marriage_id = 10000;
...
foreach ($anggota as $row) {
    if ($row['ayah'] && $row['ibu']) {
        $ayah_result = $conn->query("SELECT id FROM anggotaatuk WHERE nama='{$row['ayah']}'");
        $ibu_result = $conn->query("SELECT id FROM anggotaatuk WHERE nama='{$row['ibu']}'");
