...
// -------------------------------------------------------------------------
// 1. INTERN FILTER AJAX: CEK VALIDASI PASSWORD (TEKS BIASA)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// AJAX Tambahan: Cek kuota grup Cerdas Cermat secara real-time
// -------------------------------------------------------------------------
...
if (isset($_GET['action_hapus_mandiri'])) {
    ...
    if ($db_pass && $input_pass === $db_pass && !empty($peserta_id)) {
        $stmt_lock = $pdo->prepare("SELECT is_locked, foto FROM mld_peserta WHERE id = ? AND majelis_id = ?");
        $stmt_lock->execute([$peserta_id, $selected_mt_id]);
        ...
        if ($peserta && $peserta['is_locked'] == 0) {
            ...
            header("Location: daftar.php?status=success_delete&mt=" . $selected_mt_id . "&p_v=" . $encoded_pass);
            exit;
        } else {
            header("Location: daftar.php?status=error_locked&mt=" . $selected_mt_id . "&p_v=" . $encoded_pass);
            exit;
        }
    }
}
    ...
    if ($db_pass && $input_pass === $db_pass) {
        ...
        if (!empty($id)) {
            ...
            if ($stmt_lock->fetchColumn() == 1) {
                header("Location: daftar.php?status=error_locked&mt=" . $selected_mt_id . "&p_v=" . base64_encode($input_pass));
                exit;
            }
        }
        ...
        if ($cc_kategori_id && in_array($cc_kategori_id, $kategori_ids)) {
            ...
            $query_check = "SELECT COUNT(*) FROM mld_peserta_lomba pl 
                            JOIN mld_peserta p ON pl.peserta_id = p.id 
                            WHERE p.majelis_id = ? AND pl.kategori_id = ? AND pl.grup_cc = ?";
            $params_check = [$selected_mt_id, $cc_kategori_id, $grup_cc];
            if (!empty($id)) {
            ...
            if ($stmt_check->fetchColumn() >= 3) {
                die("Akses ditolak: Kuota Grup " . $grup_cc . " untuk cabang Cerdas Cermat di Majelis Anda sudah penuh (Maksimal 3 peserta).");
            }
        }
        ...
        if (empty($id)) {
            ...
            $stmt->execute([$selected_mt_id, $nama_peserta, $no_hp, $foto_name]);
            $peserta_id = $pdo->lastInsertId();
        } else {
            $stmt = $pdo->prepare("UPDATE mld_peserta SET nama_peserta = ?, no_hp = ?, foto = ? WHERE id = ? AND majelis_id = ?");
            $stmt->execute([$nama_peserta, $no_hp, $foto_name, $id, $selected_mt_id]);
            $peserta_id = $id;
        ...
        header("Location: daftar.php?status=success&mt=" . $selected_mt_id . "&p_v=" . base64_encode($input_pass));
        exit;
    } else {
...
// Ambil list peserta milik MT terpilih beserta relasi lombanya
$selected_mt = $_GET['mt'] ?? '';
$data_peserta = [];
if (!empty($selected_mt)) {
    // Menarik ID Kategori untuk kebutuhan penandaan klasifikasi filter JavaScript di elemen HTML
    $stmt_p = $pdo->prepare("SELECT p.*, 
...
<div class="container mb-5">
    ...
    <?php if(($_GET['status'] ?? '') == 'success'): ?>
        <div class="alert alert-success alert-dismissible fade show shadow-sm" role="alert">
            <i class="bi bi-check-circle-fill"></i> Data peserta dan pilihan cabang lomba berhasil disimpan ke dalam sistem Kafilah Anda.
    ...
    <?php elseif(($_GET['status'] ?? '') == 'error_locked'): ?>
        <div class="alert alert-danger alert-dismissible fade show show shadow-sm" role="alert">
        ...
        <div class="card-body p-4">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4" id="pass_box">
                    ...
                    <input type="password" id="mt_password" class="form-control form-select-lg" placeholder="Sandi dari Panitia">
                </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-bold text-dark">Daftar Peserta Terdaftar</h4>
    ...
    <!-- FITUR BARU: Filter Cabang Lomba -->
    ...
    <!-- LIST TAMPILAN UTAMA PESERTA -->
    <div id="container_peserta">
        <?php if(count($data_peserta) == 0): ?>
            <div class="col-12 text-center py-5">
                <i class="bi bi-person-exclamation fs-1 text-muted"></i>
                <p class="text-muted mt-2">Belum ada peserta dari MT ini. Silakan klik Tambah Peserta.</p>
            </div>
        <?php else: ?>
            
            <!-- BLOK 1: Mode Normal (Tampil saat Semua Cabang / Selain Cerdas Cermat dipilih) -->
            ...
            <!-- BLOK 2: Mode Khusus Cerdas Cermat (Mengelompokkan per Grup 1,2,3,4) -->
                    ...
                    <div class="grup-container mb-4" id="box_tampilan_grup_<?= $g ?>">
                        ...
                        <div class="row g-3">
                            ...
                            foreach($data_peserta as $p) {
                                ...
                                // Cek apakah record saat ini memiliki relasi ke Cerdas Cermat dan berada di Grup $g
...
<div class="modal fade" id="modalPesertaMandiri" data-bs-backdrop="static" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <form id="formInputPeserta" method="POST" action="daftar.php" enctype="multipart/form-data" class="modal-content border-0 shadow">
...
<script>
    ...
    // FITUR BARU: Manajemen Filter Tampilan Cabang via DOM Javascript
    function filterCabang(kategoriId) {
        // Atur status tombol filter aktif
        ...
        if(kategoriId == ID_CERDAS_CERMAT) {
            // Mode Khusus: Sembunyikan daftar normal, munculkan daftar terkelompok per Grup CC
    ...
    document.getElementById('formInputPeserta').addEventListener('submit', function(e) {
        ...
        if (chkCC && chkCC.checked) {
            ...
            const mtId = document.getElementById('pstMajelisId').value;
            ...
            fetch('daftar.php', { method: 'POST', body: checkData })
                .then(res => res.json())
                .then(data => {
                    if(!data.available) {
                        alert(`Gagal menyimpan! Grup ${grup} untuk cabang Cerdas Cermat pada Majelis Taklim Anda sudah penuh (Maksimal 3 peserta). Silakan pilih grup lainnya.`);
                    } else {
                        btn.disabled = true;
    ...
    function verifyAccess() {
        ...
        .then(data => {
            if(data.valid) {
                window.location.href = 'daftar.php?mt=' + mtId + '&p_v=' + btoa(pass);
            } else {
                alert(data.msg);
            }
        });
    ...
    function pemicuHapus(id, nama) {
        ...
        if (confirm(`Apakah Anda yakin ingin menghapus peserta bernama "${nama}"? Data yang terhapus tidak dapat dikembalikan.`)) {
            window.location.href = `daftar.php?action_hapus_mandiri=1&id=${id}&mt=${mtId}&p_v=${btoa(pass)}`;
        }
    }
            ...
            fetch('peserta.php?ajax_get_lomba=1&peserta_id=' + data.id)
                ...
                .then(ids => {
                    if(Array.isArray(ids)) {
                        ...
                        // Isi data drop-down grup jika record yang diedit mengikuti lomba CC
